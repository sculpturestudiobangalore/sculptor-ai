...
type SplitInvoiceCardProps = {
    ...
    onSubmit: (data: {
        ...
        groupAGstStatus: boolean
        groupBGstStatus: boolean
    }) => void
    onCancel?: () => void
}

export default function SplitInvoiceCard({
    invoiceNumber,
    clientName,
    projectName,
    items,
    onSubmit,
    onCancel,
}: SplitInvoiceCardProps) {
    const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set())
    const [groupAGst, setGroupAGst] = useState(true)
    const [groupBGst, setGroupBGst] = useState(true)
    ...
    const handleSubmit = () => {
        ...
        onSubmit({
            ...
            splitStrategy: 'create_two_new',
            groupAGstStatus: groupAGst,
            groupBGstStatus: groupBGst,
        })
    }
    ...
    return (
        <div className="w-full rounded-2xl border border-violet-500/30 bg-linear-to-br from-violet-900/20 to-purple-900/20 p-4 shadow-xl backdrop-blur-sm sm:p-6">
                ...
                <div>
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-lg font-bold text-white">
                            Split Invoice {invoiceNumber}
                        </h3>
                    </div>
                    <p className="mt-1 text-sm text-white/70">
                        Select items to move to a <strong>new invoice</strong>.
                        <br />
                        The remaining items will stay in another new invoice.
                    </p>
                </div>
                {onCancel && (
                    <button
            ...
            {/* Items List */}
            <div className="mb-6 space-y-2 max-h-[40vh] overflow-y-auto pr-1 custom-scrollbar">
                {items.map((item) => {
            ...
            <div className="mb-6 grid grid-cols-2 gap-4">
                <div className="rounded-xl border border-white/10 bg-white/5 p-3">
                    <p className="mb-2 text-xs font-medium text-violet-300">Invoice A (Selected)</p>
                    <label className="flex cursor-pointer items-center gap-2">
                        <div
                            onClick={() => setGroupAGst(!groupAGst)}
                            className={cn(
                                "flex h-5 w-5 items-center justify-center rounded border transition-colors",
                                groupAGst ? "border-violet-400 bg-violet-500 text-white" : "border-white/30"
                            )}
                        >
                            {groupAGst && <CheckSquare className="h-3.5 w-3.5" />}
                        </div>
                        <span className="text-sm text-white/80">Apply GST</span>
                    </label>
                </div>
                <div className="rounded-xl border border-white/10 bg-white/5 p-3">
                    <p className="mb-2 text-xs font-medium text-white/50">Invoice B (Remaining)</p>
                    <label className="flex cursor-pointer items-center gap-2">
                        <div
                            onClick={() => setGroupBGst(!groupBGst)}
                            className={cn(
                                "flex h-5 w-5 items-center justify-center rounded border transition-colors",
                                groupBGst ? "border-violet-400 bg-violet-500 text-white" : "border-white/30"
                            )}
                        >
                            {groupBGst && <CheckSquare className="h-3.5 w-3.5" />}
                        </div>
                        <span className="text-sm text-white/80">Apply GST</span>
                    </label>
                </div>
            ...
            <div className="space-y-3">
                <div className="rounded-lg bg-black/20 p-3 text-xs text-white/60">
                    <p>
                        <span className="text-violet-300">New Invoice A:</span> {selectedCount} items
                    </p>
                    <p>
                        <span className="text-white/50">New Invoice B:</span> {remainingCount} items
                    </p>
                    <p className="mt-2 text-white/40 italic">
                        Original invoice {invoiceNumber} will be marked as split/cancelled.
                    </p>
                </div>
