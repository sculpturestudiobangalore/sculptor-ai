...
export class VendorManager {
  ...
  static async getVendorAvailability(
    vendorId: string
  ): Promise<{ available: boolean; next_available?: Date; reason?: string }> {
  ...
  static async findVendorForTask(taskName: string): Promise<Vendor | null> {
    ...
    const suitableVendors = allVendors.filter(
      (vendor) => vendor.type === taskType && vendor.is_available
    );

    // Return the most reliable vendor
    return (
      suitableVendors.sort(
  ...
  private static enrichVendorData(vendor: any): Vendor {
    ...
    const reliability_score =
      this.defaultReliability[vendor.name?.toLowerCase()] || 80;
  ...
  private static getVendorType(vendor: any): string {
    ...
    if (
      vendor.materials_supplied?.some(
        (mat: string) =>
          mat.toLowerCase().includes("clay") ||
          mat.toLowerCase().includes("mold")
      )
    ) {
      return "molding";
    }
    ...
    if (
      vendor.materials_supplied?.some(
        (mat: string) =>
          mat.toLowerCase().includes("brass") ||
          mat.toLowerCase().includes("metal")
      )
    ) {
      return "brass";
    }
    if (
      vendor.materials_supplied?.some(
        (mat: string) =>
          mat.toLowerCase().includes("paint") ||
          mat.toLowerCase().includes("lacquer")
      )
    ) {
      return "painting";
    }
  ...
  static async recordVendorInteraction(
    vendorId: string,
    interaction: {
