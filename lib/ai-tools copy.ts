...
export const generateContextAwarePlanTool = tool({
  description: 'Generate daily work schedule that considers current constraints, team availability, and equipment status',
  inputSchema: z.object({
    date: z.string().optional().describe('Work date (YYYY-MM-DD)'),
    teamMember: z.enum(['dhanush', 'john', 'all']).optional().default('all'),
    focusArea: z.enum(['deadline', 'payments', 'materials', 'all']).optional().default('all'),
    considerConstraints: z.boolean().optional().default(true).describe('Whether to use remembered constraints')
  }),
  execute: async (params) => {
    const scheduler = new ContextAwareScheduler()
    return await scheduler.generateContextAwarePlan(params)
  }
})

export const setContextConstraintTool = tool({
  description: 'Set temporary constraints or preferences that the AI should consider for scheduling and decisions',
  inputSchema: z.object({
    constraintType: z.enum(['team_availability', 'equipment_status', 'client_priority', 'material_delay', 'custom']),
    constraintKey: z.string().describe('Specific constraint identifier'),
    constraintValue: z.string().describe('Constraint details'),
    expiresInHours: z.number().optional().describe('How long to remember this constraint (default: 24)'),
  }),
  execute: async ({ constraintType, constraintKey, constraintValue, expiresInHours = 24 }) => {
    const expiresAt = new Date(Date.now() + expiresInHours * 60 * 60 * 1000).toISOString()

    const { data, error } = await supabase
      .from('ai_context_memory')
      .insert({
        context_type: 'scheduling_constraint',
        context_key: `${constraintType}:${constraintKey}`,
        context_value: {
          type: constraintType,
          key: constraintKey,
    ...
    return {
      success: true,
      message: `✅ Context set: ${constraintKey} = ${constraintValue}`,
      constraint: data,
...
export const interpretConstraintTool = tool({
  description: 'Interpret natural language constraints and convert them to structured context',
  inputSchema: z.object({
    userMessage: z.string().describe('User message about constraints or preferences')
  }),
  execute: async ({ userMessage }) => {
    // Simple NLP - you can enhance this with more AI later
    const message = userMessage.toLowerCase()
    
    let constraintType = 'custom'
    let constraintKey = 'general'
    let constraintValue = userMessage

    if (message.includes('sick') || message.includes('unavailable') || message.includes('leave')) {
      constraintType = 'team_availability'
      if (message.includes('john')) constraintKey = 'john_availability'
      else if (message.includes('dhanush')) constraintKey = 'dhanush_availability'
      else constraintKey = 'team_availability'
    }
    else if (message.includes('broken') || message.includes('not working') || message.includes('down')) {
      constraintType = 'equipment_status'
      if (message.includes('polish')) constraintKey = 'polishing_machine'
      else if (message.includes('grind')) constraintKey = 'grinding_machine'
      else if (message.includes('cast')) constraintKey = 'casting_equipment'
    }
    else if (message.includes('urgent') || message.includes('priority')) {
      constraintType = 'client_priority'
      // Extract client name - simple version
      const clients = ['client', 'customer', 'project']
      const foundClient = clients.find(client => message.includes(client))
      constraintKey = foundClient ? `${foundClient}_priority` : 'general_priority'
    }
    else if (message.includes('delay') || message.includes('waiting for') || message.includes('out of')) {
      constraintType = 'material_delay'
      const materials = ['clay', 'resin', 'metal', 'paint', 'material']
      const foundMaterial = materials.find(material => message.includes(material))
      constraintKey = foundMaterial ? `${foundMaterial}_delay` : 'material_delay'
    }
    ...
    return {
      interpretedConstraint: {
        ...
        key: constraintKey,
        value: constraintValue,
        confidence: 'medium' // Could be low/medium/high based on match quality
      },
      suggestion: `Use setContextConstraint with type: "${constraintType}", key: "${constraintKey}", value: "${constraintValue}"`
    }
  }
})

export const getActiveConstraintsTool = tool({
  description: 'Get all active constraints and context that the AI should consider',
  inputSchema: z.object({}),
  execute: async () => {
    const { data, error } = await supabase
      .from('ai_context_memory')
    ...
    const constraints = data?.map(item => ({
      type: item.context_value.type,
      key: item.context_value.key,
      value: item.context_value.value,
      setAt: item.context_value.setAt,
      expiresAt: item.context_value.expiresAt
    })) || []
...
class ContextAwareScheduler {
  ...
  public async generateContextAwarePlan(params: { 
    date?: string; 
    teamMember?: 'dhanush' | 'john' | 'all';
    focusArea?: 'deadline' | 'payments' | 'materials' | 'all';
    considerConstraints?: boolean;
  }) {
    const { considerConstraints = true, ...otherParams } = params;
    
    try {
      // Use the base scheduler to get the normal plan
      const basePlan = await this.baseScheduler.generateDailyPlan(otherParams);
      
      // If no constraints needed, return base plan
      ...
      if (constraints.length === 0) {
        return {
          ...
          context: {
            constraints: [],
            constraintsCount: 0,
            note: 'No active constraints found'
          }
        };
      }

      // Apply constraints to the daily plan
      const constrainedPlan = this.applyConstraintsToPlan(basePlan, constraints);

      return constrainedPlan;

    } catch (error) {
      console.error('Error in generateContextAwarePlan:', error);
      throw new Error(`Failed to generate context-aware plan: ${getErrorMessage(error)}`);
    }
  }

  private applyConstraintsToPlan(plan: any, constraints: any[]): any {
    const constrainedTasks = this.applyConstraints(plan.dailyPlan || [], constraints);
    ...
    return {
      ...plan,
      dailyPlan: constrainedTasks,
      summary: {
        ...plan.summary,
        totalTasks: constrainedTasks.length,
        activeConstraints: constraints.length,
        constraintsApplied: constraintInfo
      },
      context: {
        constraints: constraintInfo,
        constraintsCount: constraints.length,
        originalTaskCount: plan.dailyPlan?.length || 0,
        removedTasks: (plan.dailyPlan?.length || 0) - constrainedTasks.length
      }
    };
  }
}
...
export const generateAIDailyPlanTool = tool({
  description: 'Generate optimized daily work schedule for sculpture studio considering project deadlines, material availability, and team skills',
  inputSchema: z.object({
    date: z.string().optional().describe('Work date (YYYY-MM-DD)'),
    teamMember: z.enum(['dhanush', 'john', 'all']).optional().default('all'),
    focusArea: z.enum(['deadline', 'payments', 'materials', 'all']).optional().default('all')
  ...
  execute: async (params) => {
    const scheduler = new SculptureScheduler()
    return await scheduler.generateDailyPlan(params)
  }
})
...
export const aiTools = {
  ...
  TeamAvailabilityTool: getTeamAvailabilityTool,
  ...
  generateDailyPlan: generateAIDailyPlanTool,
  ...
  // Context-Aware System Tools
  setContextConstraint: setContextConstraintTool,
  getActiveConstraints: getActiveConstraintsTool,
  clearConstraint: clearConstraintTool,
  generateContextAwarePlan: generateContextAwarePlanTool,
  interpretConstraint: interpretConstraintTool,
